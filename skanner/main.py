...
def run_custom_scanner(repo_path: str, report_path: str) -> dict:
    """Run custom scanner utility with output to file."""
    try:
        # Replace 'scanner' with the actual name of your scanner binary
        cmd = [
            "scanner",
            "--dir", repo_path,
            "--json",
        ...
        # Check the return code. Adjust based on your scanner's behavior.
        if result.returncode != 0:  # Assume 0 = success, non-zero = failure
            return {
                "success": False,
                "error": f"Scanner failed with error: {result.stderr}",
            }

        # Save the JSON output to the specified report file
        try:
            ...
            return {
                "success": False,
                "error": f"Failed to write report file: {e}",
            }
...
async def scan_secrets(repo_data: RepoURL):
    """Scan endpoint that uses file output from the custom scanner."""
    ...
    report_path = os.path.join(temp_dir, "scanner_report.json")
    ...
    try:
        ...
        # Run scan with file output using the custom scanner
        scan_result = run_custom_scanner(temp_dir, report_path)
        ...
        try:
            ...
            secrets = [
                Secret(
                    description=item.get("description", "Potential secret detected"),  # Default description
